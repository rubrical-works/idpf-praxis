...
| `--prior-art` | No | Forwarded verbatim to the re-review, which runs the sweep. `/resolve-review` performs no sweep itself |
...
- After all findings are resolved, automatically triggers a re-review via `/review-issue #N --force` — appending `--prior-art` when that flag was passed to this command — and reports the updated recommendation
