...
- **`--serve` needs no `index.html`.** The reported root URL is browsable as-is: the helper serves a directory's `index.html` when one exists and otherwise renders a generated listing. A mockup set has no index (it gets a `README.md`), which is exactly the case the listing covers, so a missing index is expected rather than an error.
