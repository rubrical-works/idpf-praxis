...
**Is targeted routing in effect right now?** With `broadcast` off, `--show` and a plain run add one line that answers this from the current state, not from the setting. It either names the live monitor (session name and process id), or says it is falling back to broadcast and why (no monitor running, a stale marker, a monitor that cannot be addressed, or two sessions with the monitor's name). If the peer list or the monitor marker cannot be read, the line says routing is **undetermined** rather than claiming it is targeted. The line matters because a monitor started after your session, or one that has since stopped, changes where announcements go without any setting changing.

**About targeted routing.** It covers what `/work`, `/review-issue`, `/resolve-review` and `/done` announce. `/qa`'s `fixtures-provisioned` notice still goes to every session. The monitor passes on what matters, such as overlapping files, but if the monitor session is holding or declining its messages nobody can tell from the sending side, and then no session hears anything. Turn `broadcast` back on if you are not actively watching the monitor.
