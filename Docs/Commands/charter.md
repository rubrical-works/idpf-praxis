...
- **One question per language or platform, per role.** For each detected language (and each detected platform, such as `mobile`) and each role it has options for (`unit`, `e2e`), you pick from the merged harness registry — the shipped registry plus your project's local one. Locally contributed options are labelled `local`; platform rows are labelled as platforms. A platform is only asked about for the roles it declares.
