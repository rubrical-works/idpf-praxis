...
- Does the code implement proper timeout handling for external service calls and database queries?
- Are retry mechanisms implemented with exponential backoff and jitter to avoid thundering herd?
- Does the code use circuit breaker patterns for dependencies that may become unavailable?
- Are graceful degradation paths implemented (fallback responses, cached data) for dependency failures?
- Does error handling avoid cascading failures (e.g., unbounded retry loops, missing bulkheads)?
- Are health check endpoints implemented that accurately reflect dependency status?
- Does the code handle partial failures in distributed operations (saga pattern, compensation logic)?
