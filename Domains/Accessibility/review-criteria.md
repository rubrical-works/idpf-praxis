...
- Do all interactive elements have appropriate ARIA roles, labels, and states?
- Is keyboard navigation fully functional (Tab, Enter, Escape, Arrow keys) for all interactive components?
- Do images, icons, and non-text content have meaningful alt text or aria-label attributes?
- Are color contrast ratios sufficient (4.5:1 for normal text, 3:1 for large text per WCAG AA)?
- Does focus management work correctly for modals, popups, and dynamic content updates?
- Are form inputs properly associated with labels using `for`/`id` or `aria-labelledby`?
- Does the code use semantic HTML elements (nav, main, header, button) instead of generic divs with click handlers?
