...
Prompt the user to choose a diagram style before generating diagrams.
```javascript
AskUserQuestion({
  questions: [{
    ...
    options: [
      { label: "drawio (Rich SVG)", description: "Generate .drawio.svg files — editable in draw.io, rich visuals, stored in Diagrams/ directory" },
      { label: "ASCII (Text-based UML)", description: "Generate text-based UML diagrams inline in the PRD markdown — renders everywhere, clean diffs, no external tooling" }
    ],
    multiSelect: false
  }]
});
```
Store the selected style for use in Phase 5.5b.
### Phase 5.5b: Diagram Generation
**Diagram type selection** applies regardless of style:
...
#### If drawio style selected:
**Load with:** `{frameworkPath}/Skills/drawio-generation/SKILL.md`
**MUST:** Generate UML diagrams as `.drawio.svg` files in the `Diagrams/` directory:
```
PRD/{PRD-Name}/Diagrams/{Epic-Name}/{type}-{description}.drawio.svg
```
#### If ASCII style selected:
Generate text-based UML diagrams **inline** in the PRD markdown document. ASCII diagrams use box-drawing characters for structure.
**Formatting rules:**
- Wrap each diagram in a code fence (` ```text ... ``` `) for monospace rendering
- Use box-drawing characters for borders and connectors -- no plain-ASCII substitutes (`+`, `-`, `|`)
- Ensure proper alignment in monospace fonts (each character occupies one column)
- Place diagrams inline in the PRD under a `### Diagrams` section per epic
**ASCII diagram type templates:**
| Type | Key Elements |
|------|-------------|
...
**No `Diagrams/` directory** is created for ASCII style -- all diagrams live in the PRD markdown file.
...
```
**Note:** When ASCII diagram style is selected, diagrams are embedded inline in the PRD markdown -- the `Diagrams/` subdirectory is not created.
**Note:** Existing flat PRDs (`PRD/PRD-{name}.md`) are grandfathered.
Create PRD document at `PRD/{name}/PRD-{name}.md`.
**Load template from `{frameworkPath}/Templates/artifacts/prd-template.md`.** Read the template file and populate placeholders with actual values.
**Graceful degradation:** If template not found, warn `"PRD template file missing, using inline fallback."` and use standard PRD structure (sections: Overview, Epics, User Stories, Diagrams, Technical Notes, Out of Scope, Dependencies, Open Questions).
...
1. Check `{frameworkPath}/IDPF-Agile/Agile-Core.md` TDD Cycle section (framework-level defaults)
...
**Load template from `{frameworkPath}/Templates/artifacts/test-plan-template.md`.** Read the template file and populate placeholders with actual values.
**Graceful degradation:** If template not found, warn `"Test plan template file missing, using inline fallback."` and use standard test plan structure (sections: Source, Test Strategy Overview, Epic Test Coverage, Integration Test Points, E2E Scenarios, Coverage Targets, Approval Checklist).
**Derivation Rules:**
...
> [user selects]
```
**If user selects 1:** Prompt for issue number, then proceed with Issue-Driven Mode workflow.
---
## Workflow (Extract Mode)
For `/create-prd extract` or `/create-prd extract <directory>`:
### Step 1: Check Prerequisites
Verify `{frameworkPath}/Skills/codebase-analysis/SKILL.md` exists on disk. Also check for `Inception/` artifacts.
**If skill not found:**
```
codebase-analysis skill not installed. Install via px-manager or ask the user to install it.
```
-> **STOP**
**If `Inception/` missing:** Warn and offer `/charter` (non-blocking).
### Step 2: Load Skill
Read `{frameworkPath}/Skills/codebase-analysis/SKILL.md` for analysis capabilities and workflow instructions.
...
Use the skill's analysis output to generate the PRD via Phase 6. Same diagram style selection (Phase 5.5a) and diagram generation workflow (Phase 5.5b) as Issue-Driven mode. Present extracted features with confidence levels for user selection before generation.
...
After user confirms (or declines), persist confirmed suggestions for px-manager discovery:
```javascript
...
```
This writes to `suggestedSkills` in `framework-config.json`. Skills already in `projectSkills` are excluded. Declined suggestions are not written.
---
