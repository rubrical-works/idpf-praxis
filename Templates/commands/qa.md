...
**Write the result** into the QA body under `**Prior Art:**` via `insertPriorArtSection` from the same helper (`.tmp-$ISSUE.md` flow), so `classifyMarker` recognizes it and a later run sees a sweep was done — never treating the marker as a reason not to sweep again.
**Verdicts:**
| Verdict | Action |
|---|---|
...
Nothing in the framework matches a described behavior to a test; pairing is a skill, so the candidate set is **derived, not guessed**:
1. **Parent** — `$PARENT`.
...
```
**Items 1–4 are emitted as visible session output, in full, before any `AskUserQuestion`, on every Outcome 3 run (#2979)** — printed, not summarized, and **never as a reply** to the verdict question's *Not yet performed* option, the branch where the report reached only those who had already said they had not checked. Writing it only to `.tmp-qa-evidence-$ISSUE.md`, or only to the issue, **does not satisfy** this: a file nobody opened is not something a person has been shown. Item 5's template is in addition to the printed report, never a replacement.
**Why the order is the contract:** the question asks a person to vouch for a check, so asking first means whoever picks *Record PASS* certifies steps they were never shown — *verdict stays human* reduced to confirming this session's account of itself. It also makes asking on a first run harmless: nothing records that a report was shown, so the order is what carries the guarantee.
...
**The question text names the numbered steps and the PASS/FAIL evidence property (#2979)** — items 1 and 3 carried into the question, **not a one-line summary**. The question may render over the earlier prose, so one merely referring back leaves the person choosing with nothing on screen; *Not yet performed* then only ends the run, the report already shown.
...
**The observation and the verdict stay human — a boundary, not a gap.** With chain, placement and fixtures in code, what remains is a session reporting on its own behavior in a live `/work` run — a self-report, the evidence-free closure #2754 produced. The person watches the prompt fire and writes the `PASS`.
