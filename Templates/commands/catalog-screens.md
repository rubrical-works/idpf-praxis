...
- Discovered screens with element counts (e.g., "Login — 5 elements")
- "All of the above"
...
|-----------|-------------------|--------------------|
| React / Next.js | `.jsx`/`.tsx`, React imports, form libs | Parse JSX for props, elements, handlers; traverse component hierarchy |
...
**Consistent output:** All specs use fields from `.claude/metadata/screen-spec-schema.json`. `framework` metadata field records the detection strategy used.
...
**Abstraction-layer tracing (CRITICAL):** Follow delegation chains to actual DOM-producing code. Classify based on rendering implementation, not wrapper API. Trust implementation over wrapper when they disagree on element type or count. Record delegation source in `componentRef`, library wrapper in `libraryComponent`.
...
**Delegation chain verification:** Trace to actual rendering code before classifying. The wrapper API shape may not match the rendered UI — read the delegated implementation.
...
| Situation | Response |
|-----------|----------|
| No UI framework detected | Suggestions → STOP |
...
| No existing specs (Update flow) | Redirect to Create flow |
