...
**Pass it under `ciResult` — one object, not top-level fields (#2764):** `buildAnnouncement({event: EVENTS.CI_RESOLVED, issues, peers, ciResult})`. **Pass `ci-watch.js` stdout unaltered — no translation (#2892).** It prints `overall` + `workflows[]` on **every** path: 2+ runs `{overall, workflows}`; one run its own fields (`found`, `conclusion`, `runId`, `name`, `url`, `duration`, `failedSteps`) plus `overall` and a one-entry `workflows`; no run `{found: false, conclusion: "no-run-found", overall: "no-run-found", workflows: []}`. `failedSteps` sits on each workflow, never top-level in the multi-run shape. `ci-resolved` renders `success`, `failure`, `cancelled`, `timeout`, `no-run-found` as what they are and refuses anything else (`shouldSend: false`, notice naming the value) rather than announcing a failure; hand-mapping the output produced #2764 and #2892. The field names are still the trap: spreading them flat is the natural reading — and was **silently** wrong: `formatCiResolved` destructures `{issues, ciResult}`, so flat fields hit its fallback and **sent** *"the outcome could not be read"*, terminally, a green run reaching every peer as unreadable with no correction possible. Reproduced twice, caught both times only because a human read the text before sending. Since #2764 `buildAnnouncement` **refuses** that payload (`shouldSend: false`, notice naming this key) instead of announcing it, matching the guard `CI_TERMINAL` has always had.
