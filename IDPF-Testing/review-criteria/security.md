...
- Does this code sanitize user input before database queries or template rendering?
- Are authentication and authorization checks applied consistently on all routes and endpoints?
- Does error handling avoid leaking stack traces, internal paths, or system state to clients?
- Are secrets, tokens, or credentials hardcoded or properly loaded from environment variables?
- Does the code use parameterized queries or prepared statements for all database operations?
- Are CORS, CSP, and other security headers configured appropriately?
- Does session management follow secure defaults (HttpOnly, Secure, SameSite flags)?
