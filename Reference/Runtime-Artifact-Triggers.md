...
If conversation contains:
  - Technology comparison or selection
  - "decided to", "going with", "chose"
  - Architecture terms (monolith, microservices, serverless)
...
If conversation contains:
  ...
  - "how to test", "test coverage", "test plan"
...
```
If conversation contains:
  - "later", "TODO", "FIXME", "hack", "workaround"
  - "technical debt", "refactor later"
  - "not ideal", "temporary", "quick fix"
...
During /prepare-release:
  If Transition/Deployment-Guide.md:
...
```
If conversation contains:
  - Feature completion for user-facing work
  - UI/UX changes
  - New user workflows
  - API changes (if external)
Then:
