...
**NEVER** `gh pmu move --force` to bypass unchecked ACs on issues you implemented. Legitimate: epic parents, external closures, branch trackers, test-plan approvals, **and QA-extracted ACs** — a `- [ ] … → QA: #N` box is an intentionally-open gate (Step 4a, Option A/#2472), not unfinished work. Moving to `in_review` past **only** such `→ QA: #N` lines is permitted; scoped strictly to `→ QA: #N` lines, **not** a general force permission. Detection: `story`/`enhancement` label AND `in_progress` this session → **HALT** if any unchecked AC lacks `→ QA: #N`; report unchecked (non-QA) count, verify via Step 4.
...
3. Add `@framework-script v0.93.0` as first line of helper JSDoc — enforced by `manifest-validation.test.js` (#1019 guard).
...
If the only unchecked ACs are QA-extracted `- [ ] … → QA: #N` lines, the plain move fails checkbox validation; use `--force` (permitted by the Step 4b QA exception — valid only when *every* unchecked AC carries `→ QA: #N`): `gh pmu move $ISSUE --status in_review --force`. QA `- [ ]` boxes stay open until their QA sub-issues close (`qa-config.json` `closurePath`); parent reaches `done` only once checked.
