...
**`--prior-art` is accepted here but consumed at 2a-iv, not by the preamble (#2752).** The preamble records it and makes no sweep decision; `decideFlagSweep` owns that. It was previously absent from the argument loop, so it fell through to the positional branch and was rejected as an issue number — `ok: false`, routed to **STOP** by the branch below, halting the review before any criterion ran. Any flag-shaped token the loop does not recognize is now collected into `unrecognizedFlags` and reported rather than being fatal, per `02-github-workflow.md`'s pass-through convention.
Parse JSON. Branches:
...
**Cap: 20 children per invocation (#2958).** Sized for a 1M context base from the per-child cost measured on eleven-child epic #2843 (`Construction/Design-Decisions/2026-09-10-epic-review-expansion.md`): one child ≈ 12k tokens (preamble envelope, 2a-ii body read+write, criteria evaluation, findings, finalize), so 20 ≈ 240k estimated, not re-measured, on top of the epic's review and resident rules. Safe because recoverable: the per-member step tasks batched after the epic's preamble tell a compaction-resumed run which children remain; without task tools an already-reviewed child still early-exits on its label. History: six under #2869, sized for a 200k window — six ≈ 75k, eleven did not fit. Children beyond the cap are **not reached**, in number order, so the same 20 are reached each run until the rest are reviewed.
...
**The roster is the epic's closing report** (Step 4): every child reviewed with verdict, every child not reached with reason — `expansion cap (20)`, `interrupted`, `error: <message>` — and the command finishing the set.
...
| `recommend` ⊘ | mode `recommend` (default), no complete marker — **no sweep, no write**. Report ⊘ with the `formatSweepAdvisory()` text, naming the runnable command, as the evidence. **Not** ❌: `--prior-art` is opt-in and rarely passed, so most issues carry no marker and ❌ would downgrade nearly every review for a sweep never meant to run automatically. **Not** ⚠️ (#2989): it needs no action, and ⚠️ made `/resolve-review` ask every cycle; the evidence string, not the symbol, keeps it distinct from a `flag-only`/`off` skip. Does not affect the recommendation |
...
```
Reviews complete: #<epic> (epic) and 20 of 22 children
  Reviewed: #101 reviewed · #102 pending · …
  Not reached: #121 — expansion cap (20) · #122 — expansion cap (20)
  Interdependence (children): <findings summary, or "none detected">
Finish the set: /review-issue #121 #122
```
...
**Resolve cap: 20 `findings-pending` children per invocation (#2958).** Sized for a 1M context base from the per-child cost measured on six children of #2843 (`Construction/Design-Decisions/2026-09-10-epic-review-expansion.md`): resolving a child ≈ 8k tokens on top of its ~12k re-review, so 20 resolved plus the 20-child re-review ≈ 400k estimated, not re-measured — within 1M with the epic and resident rules. Recovery as § 2a-iii: per-member tasks batched after the epic's preamble. Matching § 2a-iii's cap keeps the single re-review covering every child the resolve pass reached. History: three under #2872, sized for a 200k window — six pending ≈ 185k, three ≈ 160k. Beyond the cap pending children are not reached, in number order; the re-review still covers § 2a-iii's 20, and the next run resumes because a resolved child re-reviews `reviewed-clean`. **The closing report is a roster:** `Resolved: #101 (3 findings) · …`, `Skipped: #103 — reviewed-clean · #104 — never-reviewed, run /review-issue #104`, `Not reached: #105 — resolve cap (20)`, the re-review's result, and `Finish the set: /resolve-review #<epic>`.
### Step 1: Setup (Preamble Script)
```bash
...
**Flagged `severity-proportionate` (#2992):** re-read `.claude/metadata/bug-priority-rubric.json`; derive its priority from the bug's stated impact as `/bug` Step 2c does (`classes`, `precedence`, no match → `fallback`). Offer it as the suggestion, naming the matched class id and the current board priority. **Accept** → `gh pmu move $ISSUE --priority <p>`; **Provide alternative** → user's `p0`/`p1`/`p2`, applied the same way. One confirmation suffices — a board field, trivially reverted. **Never auto-fix it:** `priority-set` fills a *missing* priority only; a set-but-wrong one is always a proposal, which is why `resolve-preamble.js` classifies it `needsUserInput`.
