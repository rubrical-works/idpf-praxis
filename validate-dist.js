...
if (manifest) {
  ...
  // Version should not contain 0.76.0 placeholder
  if (manifest.version && !manifest.version.includes('{{')) {
    pass(`Version resolved: ${manifest.version}`);
  } else if (manifest.version) {
    fail('Version contains unresolved 0.76.0 placeholder');
  }
}
...
for (const file of sampleFiles) {
  ...
  if (fs.existsSync(fullPath)) {
    ...
    if (content.includes('0.76.0')) {
      fail(`${file} — contains unresolved 0.76.0 placeholder`);
    } else {
