...
New this release: `/bug` sets a starting priority from the bug's described impact, using a shared
rubric that `/review-issue` and `/resolve-review` also use. Bug reviews now check that a root cause
is identified and that a regression test is required. Commands that create or edit issues give
their scratch files per-invocation or per-issue names, so two sessions in one working directory no
longer overwrite each other's issue bodies. Reviews are shorter: five low-value criteria are
retired, story acceptance criteria are evaluated once, and the prior-art advisory is no longer
scored as a finding.

**Upgrade notes:**

- **Five review criteria are retired (#2986, #2987).** `story-sizing`, `effort-estimate`,
  `labels-correct`, `user-story-format` and `canonical-template` no longer appear in reviews.

- **Bug reviews ask for more (#2993).** A new `root-cause-identified` criterion, and
  `test-coverage-bug` now requires a regression-test acceptance criterion. Re-reviewing an existing
  bug can return findings that its earlier review did not.

- **The prior-art advisory shows ⊘, not ⚠️ (#2989).** Under the default `reviewSweep: recommend`,
  an unswept issue is scored ⊘ with the runnable `--prior-art` command as its evidence.
  `/resolve-review` no longer asks about ⊘ findings.

### Added

- `/bug` derives the issue's priority from `.claude/metadata/bug-priority-rubric.json` (ten impact
  classes, P2 when none matches), and `/resolve-review` proposes the rubric priority for a flagged
  `severity-proportionate` finding (#2992)
- `root-cause-identified` bug review criterion (#2993)
- `/prepare-beta` warns when GitHub's Latest release is a prerelease-shaped tag (#2950)

### Changed

- `/bug`, `/enhancement` and ten other commands write issue bodies and scratch output to
  per-invocation paths, and the shell-safety rule teaches that convention (#2980, #2981, #2985,
  #2991)
- `/qa`'s manual-only outcome prints the steps before asking for a verdict (#2979)
- `severity-proportionate` judges a bug's priority against the shared rubric (#2992)
- `test-coverage-bug` requires a regression-test acceptance criterion (#2993)
- Prior-art `recommend` is scored ⊘; `/resolve-review` routes ⊘ findings to passed and keeps an
  unrecognized status in front of the user (#2989)
- Framework text uses American English throughout; six contract identifiers are renamed, including
  `recognizedGateTokens` and `recognizedForms` (#2978)
- The framework's `package.json` version now tracks the release (#2954)

### Fixed

- `/create-branch`, `/add-story` and `/split-story` no longer let concurrent sessions overwrite each
  other's issue-body edits (#2983)
- `qa-extract.js` accepts an array `steps` fill and reports fill fields it rejects (#2976)
- Story reviews no longer evaluate acceptance criteria twice (#2988)
- `announce.js` records an announcement it decides not to send as skipped, with the reason,
  rather than as pending dispatch (#2990)
- (maintainer) The spell-carry verifier works at CI's shallow clone depth (#2984)

---

## [0.105.0] - 2026-09-21

New this release: `/create-prd` can draw PRD diagrams with the optional `diagram-design` skill when
it is installed. `/x-session-config` now reports whether announcements are actually routed to a live
`/overwatch` or have fallen back to broadcast. `/done`'s push announcements are routed and recorded
like every other announcement. `/merge-branch`, `/prepare-beta`, `/prepare-release` and
`/destroy-branch` tell every session before they do anything irreversible, and `/overwatch` has its
own settings file.

**Upgrade notes:**

- **The `/overwatch` marker moved to `.claude/.overwatch/` (#2957).** A marker at the old root path
  `.overwatch.json` is still read for one release. `/overwatch --on`, `--off` and `--show` manage its new per-project settings file in the same
  directory.

- **Branch operations always notify every session (#2960).** The four branch commands announce
  before their first irreversible step, even when `broadcast: false` routes other announcements to
  `/overwatch` alone. Only turning messaging off entirely (`enabled: false`, `IDPF_X_SESSION=off`,
  or `discovery: false`) suppresses them.

### Added

- `/create-prd` offers a third diagram style, `diagram-design` (editorial SVG), when that skill is
  detected in the project, user skills or plugin registry; otherwise it names the optional skill
  and its guide (#2969)
- `/x-session-config` reports effective announcement routing: targeted to a live monitor, fallback
  to broadcast with the reason, or undetermined (#2971)
- `/overwatch` per-project config with `--on`, `--off` and `--show` (#2957)
- Branch-operation announcements from `/merge-branch`, `/prepare-beta`, `/prepare-release` and
  `/destroy-branch`, sent as a forced broadcast and recorded in the ledger (#2960)
- `/qa` generates a locally producible artifact before a manual hand-off and names where to
  inspect it (#2973)

### Changed

- `/done` composes its push-group announcements through `announce.js`, so they follow targeted
  routing and get `/overwatch` receipts (#2972)
- `/qa`'s manual-only report gives numbered steps, an observable PASS/FAIL property and the exact
  location to inspect (#2973)
- `/work` skips closed sub-issues of an epic or branch tracker without a per-child lookup (#2970)
- `/review-issue` and `/resolve-review` epic-expansion caps raised from six and three to 20
  children (#2958)
- `/idpf-stats` lists `--daily-log` first and reports every date-range flag it ignores there (#2962)
- The `ac-verification-mechanism-feasible` review criterion no longer flags API-level choices
  within a mechanism the project already uses (#2956)
- Cross-session messaging reference material moved out of rule 03 into
  `Reference/Cross-Session-Messaging.md`, and `/x-session-config` and `/overwatch` point there
  instead of restating it (#2945, #2967)
- (maintainer) CI runs change-scoped Jest on branch pushes and keeps the full suite at the merge
  gate (#2959); the release coverage gate runs through `test:coverage` with a declared heap (#2953)

### Fixed

- `/idpf-stats` stamps each date with that date's own UTC offset across a DST change (#2964)
- `/prepare-release`'s post-tag CI wait waits for GitHub to register the tag's run, and stops
  rather than passing when an expected run never appears (#2952)
- (maintainer) `upstream-monitor.test.js` no longer reads the real project root (#2938);
  `manifest-validation.test.js` treats `.claude/.overwatch/` as not deployed (#2968)

---

## [0.104.0] - 2026-09-18

New this release: `/hall-monitor` is now `/overwatch`, and it does more than watch. It confirms
receipt of each announcement routed to it, tells two sessions when their issues declare the same
files, and flags a final announcement that a later event contradicts. Setting `broadcast: false`
routes announcements to a live `/overwatch` alone. `/bluf` gives a short conclusion-first brief for
one or more issues, `/idpf-stats --daily-log` writes a day's work log for a manager to read, and
session startup now reports hooks that cannot load or keep failing.

**Upgrade notes:**

- **Requires Praxis Hub Manager v0.22.0-beta.2 or later (#2942).** The five command lists in
  `framework-manifest.json` are replaced by one `deploymentFiles.commands.registry`, and the
  manifest declares `consumerCompatibility.praxisHubManager.manifestContract: 2`. An earlier Praxis
  Hub Manager reads only the retired keys and can install this release with no commands while
  reporting success. v0.22.0-beta.2 reads the registry (px-manager#1254) and still serves earlier
  framework versions (px-manager#1255).

- **`/hall-monitor` is renamed `/overwatch` (#2928).** Invoke `/overwatch`. Its marker file is now
  `.overwatch.json`; an existing `.hall-monitor.json` is still read.

- **Two new cross-session levers in `.claude/x-session.json` (#2914, #2915).** `broadcast: false`
  sends announcements only to a live `/overwatch`, falling back to broadcast when none is found;
  `overlapNotices` controls the overlap notices `/overwatch` sends. Both default to on when absent,
  so nothing changes until you set them. `/x-session-config` shows and toggles both.

- **Framework code moved out of `USER-EXTENSION` blocks (#2913).** `/create-branch`'s config-clean
  check and `/destroy-branch`'s workstream detection shipped inside extension blocks; they now sit
  outside them, and the blocks ship empty. A project copy still carrying that text inside its
  blocks runs the step twice — clear it from the block.

- **`/idpf-stats --save` writes to `Construction/Reports/Stats/` (#2925).** `/charter` creates
  `Construction/Reports/{Daily-Logs,Stats}` at inception, and `/charter refresh` offers them.

### Added

- `/overwatch` receipt replies: the sender records `received` for an announcement only when a
  receipt arrives (#2922)
- `/overwatch` overlap notices to sessions whose in-flight issues declare the same files (#2914)
- `/overwatch` `terminal-claim-contradicted` signal for a final announcement a later event
  contradicts (#2902)
- Targeted announcement routing with broadcast fallback, `announce-routing.js` (#2915)
- `/bluf` — a short conclusion-first brief per issue, keyed to the issue's type (#2932, #2935)
- `/idpf-stats --daily-log` and `daily-log-collect.js` (#2925)
- Hook heartbeats (`lib/hook-heartbeat.js`) and the startup Hook Health row (#2917)
- `consumerCompatibility` contract declaration in `framework-manifest.json` (#2942)

### Changed

- Every command description rewritten as a complete sentence naming IDPF (#2908)
- `/hall-monitor` renamed `/overwatch` across the command, libraries, metadata, marker and docs
  (#2928)
- Command deployment is declared once in `deploymentFiles.commands.registry`, with `marker` and
  `env` per command (#2942); `/audit-commands` and `/prepare-release` validation read it (#2946)
- Startup passes its `gh pmu` version to the config-integrity check instead of running it twice
  (#2907)
- `/prepare-release`'s Summary Checklist no longer carries per-feature manual steps (#2940)
- (maintainer) `/fw-minimize-files` carries frontmatter-only command edits forward without
  regeneration (#2921); the release gate checks that no framework-owned path ships in a deployed
  command (#2943); a hands-on probe of `PostToolUse` `updatedToolOutput` and `PostToolBatch` is
  recorded (#2918); proposal: CHARTER and Inception Content Ownership (#2937)

### Fixed

- `package.json` declares Apache-2.0, matching `LICENSE`, `NOTICE` and the READMEs (#2916)
- Rule 02 no longer licenses raw `gh pmu create` in place of `/bug` and `/enhancement` (#2934)
- `work-completed` names only the current branch's commits (#2888)
- `ci-resolved` refuses an unrecognized CI outcome instead of announcing a failure (#2892)
- The announcement ledger records the session's pid, not a child process's (#2896)
- `/done` detects a push trigger declared with `paths` only, instead of skipping CI (#2897)
- `/prepare-beta`'s CI wait is scoped to the beta branch (#2889)
- `/overwatch --force` no longer claims a displaced monitor is running when liveness is
  unconfirmed (#2868)
- `/charter` entity-count verification no longer reports false mismatches for file locations and
  exclusion prose (#2919)
- (maintainer) `validate-helpers.js` and release validation no longer read the retired command-list
  keys (#2946, #2948); rule 04's hook counts and runtime dependencies corrected (#2920); two Stage 2
  audience tests read a fixture instead of git history, with a guard against the pattern (#2947)

### Removed

- Gemini CLI compatibility: `/fw-transform-gemini`, its tests and every live reference; the
  Gemini CLI Extension Packaging proposal is withdrawn (#2941, #2111)
...
- Charter template detection recognizes the Praxis Hub Manager bootstrap charter (#2893)
...
run, and `/domain-config` reads and edits `activeDomains`. Reviewing or resolving an epic now treats
the epic and its stories as one set, children first, so no story is labeled reviewed without being
reviewed. Cross-session settings move to a per-developer `.claude/x-session.json`, and session
startup clears stale `.tmp-*` scratch files.

**Upgrade notes:**

- **Cross-session settings move to `.claude/x-session.json` (#2774).** The `crossSessionMessaging`
  key in `framework-config.json` is deprecated but still read as a fallback; the next
  `/x-session-config` write moves it across. `IDPF_X_SESSION=off` mutes emission for one session
  without touching either file (#2705), and a live `/hall-monitor` quiets inbound narration for its
  lifetime (#2769).

- **QA sub-issues get their own board status (#2821).** They are placed on the parent's branch in a
  `QA Required` column, aliased `qa_required`; add that status option to the project board so the
  alias resolves. `/work` hands a single `qa-required` issue to `/qa` and skips such sub-issues in an
  epic or branch-tracker run (#2824).

- **Epic reviews expand into their stories (#2869, #2870, #2872).** `/review-issue` on an epic
  reviews each child as its own issue (up to six per run), then the epic, whose `sub-issue-review`
  verdict records every child's outcome. The epic's label no longer propagates to its children, and a
  `reviewed` label with no `**Reviews:**` marker now classifies as never reviewed. `/resolve-review`
  on an epic resolves findings-pending children first (up to three per run) and re-reviews once.

- **Startup removes stale `.tmp-*` files (#2771)** — untracked regular files a day old or older. Opt
  out with `tmpCleanup: false` in `framework-config.json`.

- **Always-protected paths are checked against the authored scope only (#2788).** A path appended
  under `### Files Changed` no longer licenses an edit to a protected file; name it under
  `**Files to modify:**` instead. A table after that declaration no longer extends it (#2813).

### Added

- `/qa` — works a QA-Required issue: an obsolete-gate sweep (#2825), board fixtures provisioned and
  torn down behind two consent gates (#2827), and verdict capture on re-invocation (#2835) (#2823)
- `/idpf-measure --start | --stop | --report | --reset` — hook-based run instrumentation grouped by
  session, disclosing truncated collection and fallback spawn cost (#2794, #2796, #2797, #2798, #2799,
  #2800, #2803, #2807)
- `/domain-config --add | --remove | --list` for `activeDomains`, with auto-inclusion source
  attribution (#2811)
- `/charter` collects verification commands and declares them in `framework-config.json` (#2805)
- `.claude/x-session.json` and `x-session.schema.json`, with install defaults held apart from read
  defaults (#2774, #2783); `IDPF_X_SESSION` session-scoped opt-out (#2705)
- Startup Trash sweep and its `tmpCleanup` opt-out (#2771)
- `review-findings` announcement for a review ending with findings, so every `review-started` opener
  is closed (#2781); `announce.js` composes all four review events and enforces the `--force`
  suppression in code (#2871)
- `announce.js` with a sender-side ledger: commit payloads are derived from git, and Step 6a
  reconciles openers against closers (#2790)
- `tree-review-gate.js`, the scripted `/work` Step 2b-ii chain, testable against a mocked board
  (#2826)
- Hall-monitor presence marker (#2769) and an `opener-while-previous-unmatched` finding kind (#2879)
- A trigger flag may claim the rest of the line, declared per flag (#2770); verbal test-plan approval
  routes to `/review-test-plan` (#2815); trigger patterns move to metadata (#2816)
- Prior-art sweep: count-first retrieval budget, Prior Art and Files Changed sections excluded from
  issue-history matching, and a truncation signal (#2874)

### Changed

- `/work` redirects a test-plan approval issue to the `/review-test-plan` gate rollup (#2784)
- `/review-test-plan` is gated on the source PRD's review state (#2786)
- Multi-issue review runs define when an issue's turn begins: opener and verdict for one issue before
  the next issue's opener (#2879)
- A branch tracker under `--nonstop` does not offer proceed-with-clean members (#2780)
- `/done` peer announcements name every issue in the pushed commit range (#2772)
- Two-phase task creation states one-message emission at every Phase 2 site, and § Measured basis
  distinguishes intervals from execution time (#2793, #2801, #2806)
- `/prepare-release` Step 3.1 requires a short, user-facing CHANGELOG lead (#2777)
- The Story Body Template drops the per-story skill list and adds a conditional E2E section (#2817)
- Review commands compose the domain `Available:` tip from the registry (#2812)
- `validate-rules-registry.js` reports a verdict matching its evidence, with tracked divergences
  (#2779)
- `minimize-helper.js help` documents every subcommand (#2789)
- Skills imported from `idpf-praxis-skills` v0.18.0; the narrowing `testCoverageAudit` override is
  removed (#2867)
- `Docs/03-Philosophy` and `Docs/04-Suitability` brought in line with the current framework, plus a
  Sequential-vs-Swarm philosophy document
- `domain-config.md` registered for deployment and `qa` classified in `managedCommands`; 18
  `minimize-targets.json` entries given their first recorded run (release validation)

### Fixed

- The always-protected scope-drift halt could be satisfied by the run's own Files Changed append
  (#2788); a table following the Files to modify declaration was harvested as declared scope (#2813)
- An epic review labeled its stories reviewed without reviewing them (#2869)
- Review numbering counted any `## … Review #` comment (#2837), and the `**Reviews:** N` marker was
  matched inside quoted prose (#2880)
- `/work --nonstop` dropped a work-started event and announced a commit SHA that never existed
  (#2790)
- `assign-branch --remove` reported the tracker unlink as done without performing it (#2814)
- `--target` filing added the issue to this repository's board rather than the companion's (#2775)
- `/done` announcements named the issue batch, not the pushed range (#2772)
- An empty `activeDomains` suppressed every domain in `/code-review` (#2810)
- `/review-test-plan` Step 5a was skipped on "Ready with minor gaps" (#2785)
- The `workflow-trigger.js` status extractor was unanchored, so prose containing "in" routed to a
  batch `/work` (#2782)
- The hall-monitor self carve-out was unreachable, so a re-arming monitor refused itself (#2795)
- Tests depended on an ambient `CLAUDE_PID` (#2809); a rules-junction guard was vacuous (#2802); an
  unreachable `changeFailureRate` tier (#2787)
- `js-yaml` override floor raised to 3.15.2 for GHSA-2883-xcg3-v3hh

---

## [0.101.0] - 2026-09-04

New this release: `/hall-monitor` dedicates a session to reading cross-session announcements,
`/proposal` gains an `--update` mode for revising an existing proposal, and `/x-session-config` gains
`--quiet` / `--loud`. `/work` now verifies against a declared `verificationCommands` set and checks
review state once, up front, across a whole epic, branch tracker or typed selection. `/review-issue`
becomes a managed command, the review workflow moves into a ninth auto-loaded rule, and the resident
`/work` rule is about a fifth smaller. Fixes cover sub-issue enumeration in `/work`, the
`--prior-art` flag on `/review-issue`, file attribution in `/done`, and `CI_RESOLVED` announcements.

Release validation caught two command-registration gaps and two stale `.min-mirror/` outputs; all
four are fixed in this release and recorded in `Construction/Test-Plans/v0.101.0-release-test-plan.md`
§ J.
...
- **`/work` verifies against a declared command set (#2733).** `framework-config.json` gains
  ...
  honored as the fallback. A project declaring neither is told the sweep did not run and the
...
- **`/x-session-config --quiet` and `--loud` (#2735)** set a new receive-side lever,
  `noticeNarration`. Quiet keeps the one-line acknowledgment of an inbound announcement — the only
...
- `noticeNarration` lever with `--quiet` / `--loud` aliases and `--show` on `/x-session-config`, plus
  a per-project memory artifact and config/memory drift reporting (#2735)
...
- `/done` derives the work-summary file list per commit rather than by positional range (#2732);
  ...
  hand-rolled alternation in favor of `issueRefGrepPattern` with keywords (#2753)
...
- `Docs/02-Advanced/Claude-Code-Dependencies.md` §2 now records compaction behavior as measured,
...
reports against labeled DORA and SPACE populations with provenance instead of bare numbers.
...
- **The TDD RED gate has a host-process carve-out (#2556).** `verificationMode: "host-process"` in
  ...
  Absent, unreadable, or unrecognized values all resolve to `automated-tests`, so a typo cannot
...
- Test-plan approval gate automation: a single declared gate set rendering both checklists, a
  ...
  (#2693, #2710, #2711, #2712)
- DORA and SPACE reporting in `/idpf-stats` with labeled benchmark populations and provenance, plus
...
- `/work` peer announcement event 2 had no `--nonstop` behavior, making "not sent" indistinguishable
...
- Design decisions recorded for the gate annotation naming its resolving event, candidate test paths
  at authoring time, and the upstream monitor's diverged behavior
...
- **`/work` and `/done` announce lifecycle events to peers (#2662, #2663).** Five events:
  ...
  awaits delivery or acknowledgment, and a failed send is reported while work proceeds. An advisory
...
- **`platformOnly` gates rule *generation*, not rule *loading* (#2650).** `CLAUDE.md` described
  ...
  projects genuinely differ — Praxis Hub Manager generates rules at install and does honor the flag.
...
Feature release, and the largest since the workflow gates were introduced. Its center of gravity
...
- **`minimize-helper orphans` recognizes the `.dev.md` side-file convention** instead of reporting
...
- Domain specialist never loaded in hub-managed projects: framework paths now resolve through a
  single `path.resolve`-based helper, so an absolute `frameworkPath` is honored instead of
  concatenated. Both call sites were centralized, because fixing one alone relocates the failure
...
- Dual publication targets, parameterized through `.github/dist-targets.json`, with a pre-flight
...
- `tests/deployment/deploy-dist-dual-target.test.js` — asserts the deploy *mechanism*: one
  parameterized definition, pre-flight ordering, single source-release, and a repo-wide sweep for
...
- **Four-mode `reviewSweep` setting** (#2564). Prior-art sweeping becomes opt-in through
  ...
  join Praxis Hub Manager as a writer of the key, materializing it through the validating writer on
...
- **`node_modules/.package-lock.json` untracked** (#2543); while tracked it misreported installed
  versions to anything reading it, and dependency-check behavior in the post-pull marker gap is now
  pinned by test.
...
- **Start script version injection** (#1956) — Added `.cmd` and `.sh` to `deploy-dist.yml` version injection step; `v0.106.0` now substituted in start scripts
...
- **`code-path-discovery.zip`** — rebuilt with version substitution (was containing `v0.106.0` placeholder)
...
- **framework-manifest.json version placeholder**: Replace hardcoded version with `v0.106.0` placeholder, matching the deployment pattern used by all other framework files (#1479)
- **generate-test-plan.js**: Handle `v0.106.0` placeholder gracefully by falling through to `vX.Y.Z` default (#1479)
- **audit.js**: Skip version mismatch check when manifest uses `v0.106.0` placeholder in dev environment (#1479)

### Added

- Manifest version validation test accepting both semver and `v0.106.0` placeholder (#1479)
...
### Fixed
- **#1059** - Skills retain v0.106.0 placeholder after packaging
  ...
  - Added v0.106.0 detection check to `/skill-validate` (Check 2.6)
...
- **#1092** - Standardize skill version format to YAML frontmatter
  - Updated all 25 skill source files to use `version: "v0.106.0"` in YAML frontmatter
...
- **#1019** - Standardized JS versioning with `@framework-script` tag
  - All 52 framework JS files now use `@framework-script v0.106.0` pattern
...
### Fixed
- **#951** - Replace hardcoded versions with `v0.106.0` placeholder
...
- **#933** - v0.106.0 tokens in 12 script files
  - Replaced hardcoded version numbers with `v0.106.0` placeholder
...
- **#900** - Fixed stale `frameworkVersion` in `framework-config.json`
  - Changed hardcoded version to `v0.106.0` placeholder
...
### Fixed
- **#887** - `framework-manifest.json` now uses `v0.106.0` placeholder for proper version injection during deployment
...
- **#847** - Tag format standardization
  ...
  - Frontmatter uses `v0.106.0` placeholder instead of hardcoded versions
  - Installer regex updated for backward compatibility
- **#840** - PRD directory structure: `PRD/Active/` and `PRD/Implemented/`
- **#821** - README-DIST.md now uses `v0.106.0` placeholder
...
- **Rules rebuild from minimized sources** - All rules now use v0.106.0 placeholder
...
- Restored v0.106.0 placeholders to 209 framework source files
...
### Fixed
- **Version placeholder handling** - `parseManifest()` now correctly handles `v0.106.0` placeholder in `Templates/framework-manifest.json`
...
- **Version placeholder standardized** - All version tokens now use `v0.106.0` format for consistent replacement
...
### Fixed
- **Template version placeholders** (#627) - Fixed 35+ Template files missing `v0.106.0` placeholder. Commands, scripts, and shell scripts now properly receive version during installation.
