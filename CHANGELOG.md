...
- **Start script version injection** (#1956) — Added `.cmd` and `.sh` to `deploy-dist.yml` version injection step; `v0.68.0` now substituted in start scripts
...
- **`code-path-discovery.zip`** — rebuilt with version substitution (was containing `v0.68.0` placeholder)
...
- **framework-manifest.json version placeholder**: Replace hardcoded version with `v0.68.0` placeholder, matching the deployment pattern used by all other framework files (#1479)
- **generate-test-plan.js**: Handle `v0.68.0` placeholder gracefully by falling through to `vX.Y.Z` default (#1479)
- **audit.js**: Skip version mismatch check when manifest uses `v0.68.0` placeholder in dev environment (#1479)

### Added

- Manifest version validation test accepting both semver and `v0.68.0` placeholder (#1479)
...
### Fixed
- **#1059** - Skills retain v0.68.0 placeholder after packaging
  ...
  - Added v0.68.0 detection check to `/skill-validate` (Check 2.6)
...
- **#1092** - Standardize skill version format to YAML frontmatter
  - Updated all 25 skill source files to use `version: "v0.68.0"` in YAML frontmatter
...
- **#1019** - Standardized JS versioning with `@framework-script` tag
  - All 52 framework JS files now use `@framework-script v0.68.0` pattern
...
### Fixed
- **#951** - Replace hardcoded versions with `v0.68.0` placeholder
...
- **#933** - v0.68.0 tokens in 12 script files
  - Replaced hardcoded version numbers with `v0.68.0` placeholder
...
- **#900** - Fixed stale `frameworkVersion` in `framework-config.json`
  - Changed hardcoded version to `v0.68.0` placeholder
...
### Fixed
- **#887** - `framework-manifest.json` now uses `v0.68.0` placeholder for proper version injection during deployment
...
- **#847** - Tag format standardization
  ...
  - Frontmatter uses `v0.68.0` placeholder instead of hardcoded versions
  - Installer regex updated for backward compatibility
- **#840** - PRD directory structure: `PRD/Active/` and `PRD/Implemented/`
- **#821** - README-DIST.md now uses `v0.68.0` placeholder
...
- **Rules rebuild from minimized sources** - All rules now use v0.68.0 placeholder
...
- Restored v0.68.0 placeholders to 209 framework source files
...
### Fixed
- **Version placeholder handling** - `parseManifest()` now correctly handles `v0.68.0` placeholder in `Templates/framework-manifest.json`
...
- **Version placeholder standardized** - All version tokens now use `v0.68.0` format for consistent replacement
...
### Fixed
- **Template version placeholders** (#627) - Fixed 35+ Template files missing `v0.68.0` placeholder. Commands, scripts, and shell scripts now properly receive version during installation.
